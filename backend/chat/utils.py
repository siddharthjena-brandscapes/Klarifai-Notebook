...
#         # Try these paths in order (with expanded possibilities):
#         paths_to_try = [
#             blob_path,  # Original path
#             f"faissindex/{os.path.basename(blob_path)}",  # Direct faissindex folder
#             f"faissindex/{blob_path}",  # Full path in faissindex folder
#             os.path.basename(blob_path),  # Just the filename
#             f"media/documents/{blob_path}",  # Add media/documents/ prefix
#             blob_path.replace("media/documents/", ""),  # Remove media/documents/ if present
#             f"media/{blob_path}",  # Add just media/ prefix
#             blob_path.replace("media/", ""),  # Remove media/ if present
#             blob_path.replace("faissindex/", "")  # Remove faissindex/ if present
#         ]
...
#             print(f"BLOB DEBUG: Trying path: {normalized_path}")
            
#             # Get the blob client
#             blob_client = blob_service_client.get_container_client(container_name).get_blob_client(normalized_path)
            
#             # Check if blob exists
#             try:
#                 if blob_client.exists():
#                     # Download the blob
#                     print(f"BLOB DEBUG: Blob found at {normalized_path}, downloading to: {temp_path}")
#                     with open(temp_path, "wb") as local_file:
#                         download_stream = blob_client.download_blob()
#                         local_file.write(download_stream.readall())
                    
#                     print(f"BLOB DEBUG: Blob downloaded successfully to {temp_path}")
#                     return temp_path
#             except Exception as path_error:
#                 print(f"BLOB DEBUG: Error trying path {normalized_path}: {str(path_error)}")
#                 continue
        
#         print("BLOB DEBUG: No matching blob found after trying all path variations")
        
#         # Last resort - try listing blobs with similar names
#         try:
#             container_client = blob_service_client.get_container_client(container_name)
#             print("BLOB DEBUG: Listing all blobs in faissindex folder...")
#             blobs_list = container_client.list_blobs(name_starts_with="faissindex/")
            
#             # Print the first 10 blobs for debugging
#             count = 0
...
#             blobs_list = container_client.list_blobs(name_starts_with="faissindex/")
#             for blob in blobs_list:
#                 if basename in blob.name:
#                     print(f"BLOB DEBUG: Found potential match: {blob.name}")
...
#                     print(f"BLOB DEBUG: Potential match downloaded to {temp_path}")
#                     return temp_path
...
# def delete_from_azure_blob(blob_path, container_name=None):
#     """
#     Delete a blob from Azure Storage (alias for delete_blob function)
...
#         bool: True if deletion was successful, False otherwise
#     """
#     return delete_blob(blob_path, container_name)
...
#             print(f"BLOB CONTENT DEBUG: Trying path: {normalized_path}")
...
#             try:
#                 if blob_client.exists():
#                     print(f"✅ Blob found at {normalized_path}")
...
#                     if blob_size == 0:
#                         print(f"❌ Blob is empty at {normalized_path}")
#                         continue
...
#                     print(f"✅ Successfully downloaded {len(content)} bytes from {normalized_path}")
#                     return content
#                 else:
#                     print(f"❌ Blob does not exist at {normalized_path}")
                    
#             except Exception as path_error:
#                 print(f"❌ Error trying content path {normalized_path}: {str(path_error)}")
#                 continue
