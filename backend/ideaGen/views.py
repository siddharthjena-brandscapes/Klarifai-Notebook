...
# HF_API_TOKEN = "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"
...
#     # Group active fields by their type
...
#     print("from function generate_ideas:", request.method)
...
#             # Convert dynamic fields to readable format
#             formatted_dynamic_fields =  generate_prompt_text(dynamic_fields) #json.dumps(dynamic_fields, indent=2)
            
#             # Generate enhanced product description
#             #decomposed_aspects = decompose_product_description(product, model)
#             #enhanced_description = synthesize_product_aspects(product, decomposed_aspects, model)
...
#             prompt = (
#                 f"Generate {number_of_ideas} unique and creative product ideas for a product named {product} under the brand {brand} with category {category}.\n"
#                 "While crafting these ideas, first analyze the following dynamic attributes provided as contextual guidelines. Verify the validity and relevance of each attribute based on the context of the product. "
#                 "If any attribute seems out of place or invalid for the product context, feel free to disregard it when formulating the idea, but ensure that the remaining relevant attributes are seamlessly integrated.\n"
...
#                 "Ensure that the description explains how the product benefits the user and what makes it special, making it easy to visualize the idea. "
#                 "While doing so, incorporate all the valid and relevant contextual cues from the dynamic attributes, and if any attribute appears invalid or irrelevant, omit it from the final narrative.\n"
#                 "Aim for a variety of ideas such that each is unique and creative, focusing on different aspects of the product and appealing to a variety of user segments.\n"
#                 "For each idea, provide a detailed explanation with a maximum of 50 words in the description."
#             )


#             print("Prompt Generated from generate_ideas function::", prompt)
...
#                                             }
#                                             )
#             print("Response Generated from generate_ideas function::", response)
#             try:
...
#             response["Access-Control-Allow-Origin"] = "http://localhost:5173"
#             response["Access-Control-Allow-Credentials"] = "true"
#             print("From update_idea:", response)
#             return response
...
#             for product_idea in product_ideas:
#                  # Get ideas ordered by their original set and position
#                 ideas = Idea.objects.filter(
#                     product_idea=product_idea
#                 ).select_related('product_idea').order_by('idea_set', 'idea_set_label')
    ...
    if request.method == 'POST':
        try:
            ...
            ideas_prompt = (
                ...
                "Aim for a variety of ideas such that each is unique and creative, and focuses on different aspects of the product, leveraging all provided information to appeal to a variety of user segments.\n"
...
@permission_classes([IsAuthenticated])
def generate_product_image(request):
    """Enhanced image generation endpoint with optimized retry mechanism and error handling"""
    if request.method == "OPTIONS":
        response = JsonResponse({})
        response["Access-Control-Allow-Origin"] = "http://localhost:5173"
        response["Access-Control-Allow-Methods"] = "POST, OPTIONS"
    ...
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            idea_data = data.get('description')
            idea_id = data.get('idea_id')
            
            # Input validation
            if not idea_data or not idea_id:
                return JsonResponse({
                    "success": False,
                    "error": "Product description and idea ID are required"
                }, status=400)
            
            # Get optional parameters with defaults
            initial_size = min(max(data.get('size', 768), 512), 1024)  # Constrain size
            initial_steps = min(max(data.get('steps', 50), 20), 100)   # Constrain steps
            guidance_scale = min(max(data.get('guidance_scale', 7.5), 1.0), 20.0)  # Constrain scale
            ...
            # Comprehensive prompt generation pipeline
            try:
                # 1. Decompose the product description
                aspects = decompose_product_description(
                    {"product_name": idea.product_name, "description": idea_data}, 
                    model
                )
                
                # 2. Synthesize aspects into enhanced description
                enhanced_description = synthesize_product_aspects(
                    {"product_name": idea.product_name, "description": idea_data}, 
                    aspects, 
                    model
                )
                
                # 3. Generate final enhanced prompt for image generation
                final_prompt = enhance_prompt(enhanced_description, model)
                
            except Exception as e:
                print(f"Error in prompt generation pipeline: {str(e)}")
                return JsonResponse({
                    "success": False,
                    "error": f"Failed to generate image prompt: {str(e)}"
                }, status=500)
            
            # Store enhanced description and prompt
            idea.enhanced_description = enhanced_description
            idea.visualization_prompt = final_prompt
            idea.save()
            
            # Generate image with advanced retry mechanism
            try:
                image, error = generate_image_with_retry(
                    hf_client,
                    final_prompt,
            ...
            try:
                ...
                # Create GeneratedImage2 instance with status tracking
                generated_image = GeneratedImage2.objects.create(
                    idea=idea,
                    prompt=final_prompt,
                ...
                return JsonResponse({
                    ...
                    "generated_image_id": generated_image.id,
                    "enhanced_description": enhanced_description,
                    "visualization_prompt": final_prompt,
                    "parameters": {
                        'size': initial_size,
                        'steps': initial_steps,
                        'guidance_scale': guidance_scale
