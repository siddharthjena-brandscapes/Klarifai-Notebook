...
const MainContent = ({
  ...
  const toggleView = (view) => {
    if (view === "summary" && localSelectedDocuments.length === 0) {
      toast.warning("Please upload a document or select at least one document to view the summary.");
  ...
  const renderSummaryView = () => {
    // If no documents are selected, show a placeholder with navigation
  ...
  useEffect(() => {
    if (selectedChat) {
      ...
      // Set conversation state with the sorted messages
      ...
      if (selectedChat.selected_documents && selectedChat.selected_documents.length > 0) {
        setLocalSelectedDocuments(
          selectedChat.selected_documents.map(doc => 
      ...
      // Set other states
      if (selectedChat.follow_up_questions) {
        setCurrentFollowUpQuestions(selectedChat.follow_up_questions);
  ...
  }, [selectedChat, setFollowUpQuestions, setSummary]);
  ...
  const handleSendMessage = async (message) => {
    ...
    try {
      ...
      if (response && response.data) {
        ...
        // Update conversation and states
